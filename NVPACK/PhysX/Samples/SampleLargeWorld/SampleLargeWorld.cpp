		...
		case CAMERA_SPEED_INCREASE:
			{
				...
				{
					if(mKeyShiftDown)
						printf("running speed exceeds the maximum speed\n");
					else
						printf("walking speed exceeds the maximum speed\n");
					return;
				}
			}
			break;
...
RAWMesh* SampleLargeWorld::createRawMeshFromMeshGeom(const PxTriangleMeshGeometry& mesh, RAWMesh &rawMesh)
{
	...
	PxVec3 center = bound.getCenter();
	PxVec3 extent = bound.getExtents();
...
void SampleLargeWorld::attachNearestObjectsToCCT()
{
	...
	PxVec3 pos = PxVec3( 0.0f, capGeom.halfHeight + capGeom.radius + boxExtent, capGeom.radius*2.0f);
