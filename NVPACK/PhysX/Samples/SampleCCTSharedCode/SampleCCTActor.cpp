...
{
	const PxControllerShapeType::Enum type	= desc.mType;
	const PxExtendedVec3& position			= desc.mPosition;
	const float slopeLimit					= desc.mSlopeLimit;
	const float contactOffset				= desc.mContactOffset;
	const float stepOffset					= desc.mStepOffset;
	const float invisibleWallHeight			= desc.mInvisibleWallHeight;
	const float maxJumpHeight				= desc.mMaxJumpHeight;
