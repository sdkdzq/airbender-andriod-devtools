...
void PhysXSample::updateSweepSettingForShapes()
{
	...
	PxU32 word3 = 0;
	bool enable = false;
	if(getActiveScene().getFlags() & PxSceneFlag::eENABLE_CCD)
	{
		word3 |= CCD_LINEAR;
		enable = true;
	}

	PxU32 count = getActiveScene().getNbActors(PxActorTypeFlag::eRIGID_STATIC | PxActorTypeFlag::eRIGID_DYNAMIC);
...
void PhysXSample::onTickPostRender(float dtime)
{
	...
	{
		printf("SPU settings request: %3d contact blocks, %3d friction blocks, %3d constraint blocks\n",nbContactBlocks,nbFrictionBlocks,nbConstraintBlocks);
		printf("currently are set:    %3d contact blocks, %3d friction blocks, %3d constraint blocks\n",oldContactBlocks,oldFBlocks,oldConstraintBlocks);
	}
...
void PhysXSample::collectInputEvents(std::vector<const SampleFramework::InputEvent*>& inputEvents)
{
	...
	DIGITAL_INPUT_EVENT_DEF(CCD,							WKEY_I,			XKEY_I,			X1KEY_I,		PS3KEY_I,		PS4KEY_I,		AKEY_UNKNOWN,	OSXKEY_I,		PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_I,			WIIUKEY_UNKNOWN		);
		...
		case CCD:
			{
				if(mScene)
				{
					...
					if (s.getFlags() & PxSceneFlag::eENABLE_CCD)
					{
						s.setFlag(PxSceneFlag::eENABLE_CCD, false);
						updateSweepSettingForShapes();
						printf("PxSceneFlag::eENABLE_CCD disabled.\n" );
					}
					else
					{
						s.setFlag(PxSceneFlag::eENABLE_CCD, true);
						updateSweepSettingForShapes();
						printf("PxSceneFlag::eENABLE_CCD enabled.\n" );
					}
				}
				break;
			}
