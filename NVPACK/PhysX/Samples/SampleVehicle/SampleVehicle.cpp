...
void SampleVehicle::createVehicles()
{
	...
	if( hasVehicleFile )
	{
		...
		PxBinaryConverter* binaryConverter = PxSerialization::createBinaryConverter(*sr);
...
void SampleVehicle::collectInputEvents(std::vector<const SampleFramework::InputEvent*>& inputEvents)
{
	...
	getApplication().getPlatform()->getSampleUserInput()->unregisterInputEvent(CCD);
	...
	else
	{
		...
		ANALOG_INPUT_EVENT_DEF(CAR_ACCELERATE_BRAKE,		GAMEPAD_DEFAULT_SENSITIVITY,	WKEY_UNKNOWN,				XKEY_UNKNOWN,				X1KEY_UNKNOWN,				PS3KEY_UNKNOWN,				PS4KEY_UNKNOWN,					GAMEPAD_LEFT_STICK_Y,	OSXKEY_UNKNOWN,	GAMEPAD_LEFT_STICK_Y,		GAMEPAD_LEFT_STICK_Y,	LINUXKEY_UNKNOWN,	WIIUKEY_UNKNOWN);
