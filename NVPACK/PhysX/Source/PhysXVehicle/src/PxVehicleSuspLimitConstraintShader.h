		...
		)
	{
		...
		for(PxU32 i=0;i<4;i++)
		{
			if(data->mSuspLimitData.mActiveFlags[i])
			{
				...
				data->mSuspLimitData.mActiveFlags[i] = false;
			}
		}
		...
		for(PxU32 i=0;i<4;i++)
		{
			if(data->mStickyTireForwardData.mActiveFlags[i])
			{
				...
				p.mods.spring.stiffness = 100000;
				p.flags = Px1DConstraintFlag::eSPRING | Px1DConstraintFlag::eACCELERATION_SPRING;
				numActive++;

				data->mStickyTireForwardData.mActiveFlags[i] = false;
			}
		}
		...
		for(PxU32 i=0;i<4;i++)
		{
			if(data->mStickyTireSideData.mActiveFlags[i])
			{
				...
				data->mStickyTireSideData.mActiveFlags[i] = false;
			}
		}
