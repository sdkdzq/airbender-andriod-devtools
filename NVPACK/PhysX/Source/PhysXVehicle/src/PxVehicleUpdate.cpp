...
void computeDirection(PxU32& rightDirection, PxU32& upDirection)
{
	...
	if(gRight == PxVec3(1,0,0) || gRight == PxVec3(-1,0,0))
	{
		rightDirection = 0;
	}
	else if(gRight == PxVec3(0,1,0) || gRight == PxVec3(0,-1,0))
	{
		rightDirection = 1;
	}
	else if(gRight == PxVec3(0,0,1) || gRight == PxVec3(0,0,-1))
	{
		rightDirection = 2;
	}
	upDirection = 0xffffffff;
	if(gUp== PxVec3(1,0,0) || gUp == PxVec3(-1,0,0))
	{
		upDirection = 0;
	}
	else if(gUp == PxVec3(0,1,0) || gUp == PxVec3(0,-1,0))
	{
		upDirection = 1;
	}
	else if(gUp == PxVec3(0,0,1) || gUp == PxVec3(0,0,-1))
	{
		upDirection = 2;
	}
}
 ...
 ProcessSuspWheelTireOutputData& outputData)
{
	...
	for(PxU32 i=0;i<4;i++)
	{
		...
		if(activeWheelState && numHits>0 && hitDistance!=0.0f)
		{
			...
			if(dx > -susp.mMaxDroop)
			{
				...
				if(dynamicHitActor && !(dynamicHitActor->getRigidBodyFlags() & PxRigidBodyFlag::eKINEMATIC))
				{
					...
					PxRigidBodyExt::addForceAtPos(*dynamicHitActor,hitForce,hitContactPoints4[i]);
				}
...
void poseWheels
(const PxVehicleWheels4SimData& wheelsSimData, const PxVehicleWheels4DynData& wheelsDynData, 
...
public:
	...
	static void update(
		const PxF32 timestep, const PxVec3& gravity, const PxVehicleDrivableSurfaceToTireFrictionPairs& vehicleDrivableSurfaceToTireFrictionPairs, 
		const PxU32 numVehicles, PxVehicleWheels** vehicles, PxVehicleWheelQueryResult* wheelQueryResults);
	...
	static void updateDrive4W(
		...
		PxVehicleDrive4W* vehDrive4W, PxVehicleWheelQueryResult* vehWheelQueryResults);

	static void updateDriveNW(
		const PxF32 timestep, 
		const PxVec3& gravity, const PxF32 gravityMagnitude, const PxF32 recipGravityMagnitude, 
		const PxVehicleDrivableSurfaceToTireFrictionPairs& drivableSurfaceToTireFrictionPairs,
		PxVehicleDriveNW* vehDriveNW, PxVehicleWheelQueryResult* vehWheelQueryResults);

	static void updateTank(
		const PxF32 timestep, 
		const PxVec3& gravity, const PxF32 gravityMagnitude, const PxF32 recipGravityMagnitude, 
		const PxVehicleDrivableSurfaceToTireFrictionPairs& drivableSurfaceToTireFrictionPairs,
		PxVehicleDriveTank* vehDriveTank, PxVehicleWheelQueryResult* vehWheelQueryResults);

	static void updateNoDrive(
		const PxF32 timestep, 
		const PxVec3& gravity, const PxF32 gravityMagnitude, const PxF32 recipGravityMagnitude, 
		const PxVehicleDrivableSurfaceToTireFrictionPairs& drivableSurfaceToTireFrictionPairs,
		PxVehicleNoDrive* vehDriveTank, PxVehicleWheelQueryResult* vehWheelQueryResults);
	...
	{
		for(PxU32 i=0;i<wheels.mNbActiveWheels;i++)
		{
...
PxVehicleDrive4W* vehDrive4W, PxVehicleWheelQueryResult* vehWheelQueryResults)
{
	...
	PxVec3 inverseInertia;
	{
		...
		inverseInertia=PxVec3(1.0f/inertia.x, 1.0f/inertia.y, 1.0f/inertia.z);
	}
	...
	const PxF32 inverseTimestep=1.0f/timestep;
	const PxF32 minLongSlipDenominator=vehDrive4W->mWheelsSimData.mMinLongSlipDenominator;
	ProcessSuspWheelTireConstData constData={timeFraction, subTimestep, gravity, gravityMagnitude, recipGravityMagnitude, false, minLongSlipDenominator, vehActor, &drivableSurfaceToTireFrictionPairs};
	...
	else
	{
		...
		//a = (v - v0)/dt
		vehActor->addForce((carChassisLinVel-carChassisLinVelOrig)*inverseTimestep, PxForceMode::eACCELERATION, false);
		vehActor->addTorque((carChassisAngVel-carChassisAngVelOrig)*inverseTimestep, PxForceMode::eACCELERATION, false);
	}

	//Pose the wheels from jounces, rotations angles, and steer angles.
	poseWheels(wheels4SimDatas[0],wheels4DynDatas[0],&wheelQueryResults[4*0],numActiveWheelsPerBlock4[0],vehActor);
	for(PxU32 i=1;i<numWheels4;i++)
	{
		poseWheels(wheels4SimDatas[i],wheels4DynDatas[i],&wheelQueryResults[4*i],numActiveWheelsPerBlock4[i],vehActor);
	}
 ...
 PxVehicleDriveTank* vehDriveTank, PxVehicleWheelQueryResult* vehWheelQueryResults)
{
	...
	ProcessSuspWheelTireConstData constData={timeFraction, subTimestep, gravity, gravityMagnitude, recipGravityMagnitude, true, minLongSlipDenominator, vehActor, &drivableSurfaceToTireFrictionPairs};
...
(const PxF32 timestep, 
 const PxVec3& gravity, const PxF32 gravityMagnitude, const PxF32 recipGravityMagnitude,
 const PxVehicleDrivableSurfaceToTireFrictionPairs& drivableSurfaceToTireFrictionPairs,
 PxVehicleNoDrive* vehNoDrive, PxVehicleWheelQueryResult* vehWheelQueryResults)
{
	PX_CHECK_AND_RETURN(
	...
	{
		poseWheels(wheels4SimDatas[j],wheels4DynDatas[j],&wheelQueryResults[4*j],numActiveWheelsPerBlock4[j],vehActor);
	}
	...
	case PxVehicleTypes::eDRIVENW:
		{
			...
			PxVehicleUpdate::updateDriveNW(
				...
				gravity,gravityMagnitude,recipGravityMagnitude,
				vehicleDrivableSurfaceToTireFrictionPairs,
				vehDriveNW, vehWheelQueryResults);
	...
	case PxVehicleTypes::eDRIVETANK:
		{
			...
			PxVehicleUpdate::updateTank(
				...
				gravity,gravityMagnitude,recipGravityMagnitude,
				vehicleDrivableSurfaceToTireFrictionPairs,
				vehDriveTank, vehWheelQueryResults);
	...
	case PxVehicleTypes::eNODRIVE:
		{
			...
			PxVehicleUpdate::updateNoDrive(					
				...
				gravity,gravityMagnitude,recipGravityMagnitude,
				vehicleDrivableSurfaceToTireFrictionPairs,
				vehDriveNoDrive, vehWheelQueryResults);
...
(const PxF32 timestep, const PxVec3& gravity, const PxVehicleDrivableSurfaceToTireFrictionPairs& vehicleDrivableSurfaceToTireFrictionPairs, 
 const PxU32 numVehicles, PxVehicleWheels** vehicles, PxVehicleWheelQueryResult* vehicleWheelQueryResults)
{
		...
		case PxVehicleTypes::eDRIVE4W:
			{
				...
				PxVehicleUpdate::updateDrive4W(					
					...
					gravity,gravityMagnitude,recipGravityMagnitude,
					vehicleDrivableSurfaceToTireFrictionPairs,
					vehDrive4W, vehWheelQueryResults);
				}
			break;
...
(const PxReal timestep, const PxVec3& gravity, const PxVehicleDrivableSurfaceToTireFrictionPairs& vehicleDrivableSurfaceToTireFrictionPairs, 
 const PxU32 numVehicles, PxVehicleWheels** vehicles, PxVehicleWheelQueryResult* vehicleWheelQueryResults)
{
	PxVehicleUpdate::update(timestep, gravity, vehicleDrivableSurfaceToTireFrictionPairs, numVehicles, vehicles, vehicleWheelQueryResults);
}
