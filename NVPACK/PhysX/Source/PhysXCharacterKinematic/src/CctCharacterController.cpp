...
//	printf("StepOffset: %f\n", StepOffset);
	...
	const PxU32 MaxIterUp = Ps::isAlmostZero(SideVector) ? MaxIter : 1;
	...
	if(PerformSidePass)
	{
		...
		if(1 && constrainedClimbingMode && volume.getType()==SweptVolumeType::eCAPSULE && !(mFlags & STF_VALIDATE_TRIANGLE_SIDE))
		{
			...
			if(sideM<capsuleRadius)
			{
				...
				doSweepTest(userData, userHitData, userObstacles, volume, sensor, 1, &NbCollisions, min_dist, filters, SWEEP_PASS_SENSOR);
