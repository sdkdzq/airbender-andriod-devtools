...
#define CM_PROFILE_ZONE( _p, _id) \
	physx::profile::DynamicallyEnabledScopedEvent<PxProfileEventSender> scopedEvent( _p.getProfileEventSender(), _id, _p.getEventContext() ); NV_TEXT_PROFILE_ZONE( _p, _id);

#define CM_PROFILE_ZONE_WITH_SUBSYSTEM( _p, subsystem, eventId ) CmProfileZone<PX_PROFILE_EVENT_FILTER_VALUE(subsystem,eventId)> __zone( _p, physx::profile::EventIds::subsystem##eventId ); NV_TEXT_PROFILE_ZONE_WITH_SUBSYSTEM( _p, subsystem, eventId );
