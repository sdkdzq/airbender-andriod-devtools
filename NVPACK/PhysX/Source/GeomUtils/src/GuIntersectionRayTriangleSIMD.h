...
#ifndef GU_INTERSECTION_RAY_TRIANGLE_SIMD_H
#define GU_INTERSECTION_RAY_TRIANGLE_SIMD_H
...
namespace Gu
{
											...
											const Ps::aos::FloatV enlarge = Ps::aos::FZero())
	{
		...
		const FloatV eps = FLoad(V_LOCAL_EPSILON);
													...
													const Ps::aos::FloatV enlarge = Ps::aos::FZero())
	{
		...
		const Vec4V eps = V4Load(V_LOCAL_EPSILON);
		// Find vectors for two edges sharing vert0
		...
		const Vec4V nEnlargeV = V4Neg(Vec4V_From_FloatV(enlarge));
		...
		const Vec4V enlargeDet =  V4Add(tmp1, Vec4V_From_FloatV(enlarge));
		const BoolV con1 = BOr(V4IsGrtr(nEnlargeV, tmp2), V4IsGrtr(tmp2, enlargeDet));//return false
		const BoolV con2 = BOr(V4IsGrtr(nEnlargeV, tmp3), V4IsGrtr(V4Add(tmp2, tmp3), enlargeDet));//return false
