...
namespace Gu
{
	...
	PX_FORCE_INLINE void CalculateBoxMargin(const Ps::aos::Vec3VArg extent, Ps::aos::FloatV& margin, Ps::aos::FloatV& minMargin)
	{
		...
		const FloatV ratio1 = Ps::aos::FLoad(0.03f);
		const FloatV ratio2 = Ps::aos::FLoad(0.15f);
...
/////////////
	...
	/*	PX_INLINE	PxTransform getTransform()	const
		...
		*/
		...
		//PX_FORCE_INLINE Ps::aos::Vec3V supportLocal(const Ps::aos::Vec3VArg dir, Ps::aos::Vec3V* support, PxU32& numContacts)const  
		//{
		//	using namespace Ps::aos;
		//	//calculate 8 points
		//	Vec3V p[8];
		//	computeOBBPoints(p);
		//	FloatV min = V3Dot(dir, p[0]);
		//	for(PxU32 i=1; i<8; ++i)
		//	{
		//		const FloatV d = V3Dot(dir, p[i]);
		//	}
		//}
