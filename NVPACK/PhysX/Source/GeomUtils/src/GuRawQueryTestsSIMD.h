...
struct RayAABBTest
{
	...
	PX_FORCE_INLINE PxU32 check(const Vec3V center, const Vec3V extents) const
	{
		...
		const BoolV andABCMasks = BAnd(BAnd(maskA, maskB), maskC);
				...
				return (_mm_movemask_ps(andABCMasks)==15);
			#else		
...
struct AABBAABBTest
{
	...
	{	
		...
		//return Ps::IntTrue;
		return V3AllGrtrOrEq(V3Add(mExtents, extents), V3Abs(V3Sub(center, mCenter)));
	}
...
{
	PX_FORCE_INLINE CapsuleAABBTest(const PxTransform& t, const PxCapsuleGeometry& c)
		: RayAABBTest(t.p-t.q.getBasisVector0()*c.halfHeight, t.q.getBasisVector0(), 2*c.halfHeight, PxVec3(c.radius))
	{}
	...
	{
		return RayAABBTest::check<true>(center, extents);
	}
};
...
struct OBBAABBTests
{
	OBBAABBTests(const PxTransform&t, const PxBoxGeometry&b)										
	{
		const Vec3V eps = V3Load(1e-6f);		// ARGH. Scale-invariant epsilon
