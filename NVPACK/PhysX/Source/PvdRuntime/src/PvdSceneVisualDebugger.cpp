...
{ 
	return mPvdDataStream && mPvdDataStream->isConnected(); 
}
...
//		NpScene* npScene = getNpScene(&mScbScene);
	...
	{		
		...
		//cloth 
		{
			...
			actorArray.resize(numActors);
			npScene->getActors(PxActorTypeFlag::eCLOTH, actorArray.begin(), actorArray.size());
			for(PxU32 i = 0; i < numActors; i++)
			{
...
void SceneVisualDebugger::sendStateDatas(Sc::ParticleSystemCore* psCore)
{
	...
	doProcess &= !scbParticleSystem->isInDeviceExclusiveModeGpu(); 
#endif
	if(doProcess)
	{		
...
{	
	...
	//VisualDebugger& sdkPvd = getSdkPvd();
	PxScene* theScene = mScbScene.getPxScene();
	mMetaDataBinding.createInstance( *mPvdDataStream, *npAggregate, *theScene );
	mPvdDataStream->flush();
}
