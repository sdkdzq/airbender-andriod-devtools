		...
		PxU16 removeTriangle(PxU16 f)
		{
			...
			for(PxU16 i=0, h = mFaces[f]; i<3; ++i)
			{
				...
				if(mHalfedges[h^1].mFace == sInvalid) // was boundary edge, remove
				{
					...
					connect(mHalfedges[h].mPrev, mHalfedges[h^1].mNext);
					connect(mHalfedges[h^1].mPrev, mHalfedges[h].mNext);

					// update vertex boundary or delete
					mVertices[v0] = mVertices[v0] == h ? sInvalid : mHalfedges[h^1].mNext;
...
void PvdMetaDataBinding::sendAllProperties( PvdDataStream& inStream, const PxAggregate& inObj )
{
	...
	inStream.setPropertyMessage( &inObj, values );
	PxU32 actorsCount = inObj.getNbActors();
	PxActor * userBuf[5];
	for (PxU32 actorCnt = 0; actorCnt <actorsCount; actorCnt += 5)
	{
		PxU32 numActorsRemaining = PxMin<PxU32>(5, actorsCount - actorCnt);
		inObj.getActors(userBuf, 5, actorCnt);

		PxArticulationLink *link = NULL;
		for (PxU32 i = 0; i < numActorsRemaining; ++i)
		{
			link = userBuf[i]->isArticulationLink();
			if( link && link->getInboundJoint() == NULL)
				inStream.pushBackObjectRef( &inObj, "Actors", &link->getArticulation() );
			else if( link == NULL )
				inStream.pushBackObjectRef( &inObj, "Actors", userBuf[i] );
		}
	}	
}
...
void PvdMetaDataBinding::detachAggregateActor( PvdDataStream& inStream, const PxAggregate& inObj, const PxActor& ownerActor )
{
	inStream.removeObjectRef( &inObj, "Actors", &ownerActor );
}
