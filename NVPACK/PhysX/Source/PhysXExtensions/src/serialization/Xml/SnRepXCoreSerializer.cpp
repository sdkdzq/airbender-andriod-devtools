...
namespace physx { 
	...
	PxRepXObject PxShapeRepXSerializer::fileToObject( XmlReader& inReader, XmlMemoryAllocator& inAllocator, PxRepXInstantiationArgs& inArgs, PxCollection* inCollection )
	{
		...
		RepXVisitorReader<PxShape> theVisitor( names, contexts, inArgs, inReader, NULL, inAllocator, *inCollection );
		...
		readAllProperties( inArgs, inReader, theShape, inAllocator, *inCollection );

		return createRepXObject(theShape);
	}
	...
	PxRepXObject PxAggregateRepXSerializer::fileToObject( XmlReader& inReader, XmlMemoryAllocator& inAllocator, PxRepXInstantiationArgs& inArgs, PxCollection* inCollection )
	{
		...
		readAllProperties( inArgs, inReader, theAggregate, inAllocator, *inCollection );
		...
		if ( inReader.gotoChild( "Actors" ) )
		{
				...
				matSuccess = inReader.gotoNextSibling() )
			{
				...
				{
					PxActor *actor = NULL;
					readReference<PxActor>( inReader, *inCollection, actor );
					REPX_REPORT_ERROR_IF( actor, PxRepXErrorCode::eInvalidParameters, "PxActorRef" );
					if(actor)
					{
				...
				else if ( 0 == physx::PxStricmp( actorType, "PxArticulationRef" ) ) 
				{
					...
					readReference<PxArticulation>( inReader, *inCollection, articulation );
					REPX_REPORT_ERROR_IF( articulation, PxRepXErrorCode::eInvalidParameters, "PxArticulationRef" );
					if(articulation)
					{
	...
	void writeFabricPhase( PxOutputStream& stream, const PxClothFabricPhase& phase )
	{
		...
		for ( ; conv->mName != NULL; ++conv )
			if ( conv->mValue == phase.phaseType ) 
				stream << conv->mName;
	...
	PxRepXObject PxClothRepXSerializer::fileToObject( XmlReader& inReader, XmlMemoryAllocator& inAllocator, PxRepXInstantiationArgs& inArgs, PxCollection* inCollection )
	{
		...
		REPX_REPORT_ERROR_IF( fabric, PxRepXErrorCode::eInvalidParameters, "Fabric" );
		if ( fabric == NULL )
			return PxRepXObject();
		...
		readAllProperties( inArgs, inReader, cloth, inAllocator, *inCollection );
	...
	PxRepXObject PxParticleRepXSerializer<TParticleType>::fileToObject( XmlReader& inReader, XmlMemoryAllocator& inAllocator, PxRepXInstantiationArgs& inArgs, PxCollection* inCollection )
	{
		...
		if (validParticleRange > 0)
		{
			...
			readAllProperties( inArgs, inReader, theParticle, inAllocator, *inCollection );
