...
namespace physx { namespace Sn {
	...
	{
		const PxBase* s =  reinterpret_cast<const PxBase*>( inDatatype ) ;
		if( inDatatype && !inCollection.contains( *const_cast<PxBase*>(s) ))
			REPX_REPORT_ERROR_IF( false, PxRepXErrorCode::eReferenceNotFound, inPropName );
	...
	void handleShapes( TVisitor& visitor, const PxRigidActorShapeCollection& inProp )
	{
		...
		if ( count )
		{
			...
			for( PxU32 idx = 0; idx < count; ++idx )
			{
				const PxShape* inProp = theData[idx];
				visitor.pushName( "PxShape" );
				
				if( !inProp->isExclusive() )
				{
					writeReference( visitor.mWriter, visitor.mCollection, "PxShapeRef", inProp );
				}
				else
				{
					handleComplexObj( visitor, inProp, theInfo );
				}
				visitor.popName();
			}
		}
	...
	struct RepXVisitorWriterBase
	{
		...
		void extendedIndexedProperty( PxU32* /*key*/, const TAccessorType& inProp, TInfoType& /*inInfo */)
		{
			...
			for(PxU32 i = 0; i < count; ++i)
			{
				...
				handleComplexObj(*this, &propVal, PxClassInfoTraits<TPropertyType>().Info);
				popName();
			}		
		}
