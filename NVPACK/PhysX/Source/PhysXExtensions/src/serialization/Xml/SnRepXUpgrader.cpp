...
namespace physx { namespace Sn {
	...
	RepXCollection& RepXUpgrader::upgrade3_2CollectionTo3_3Collection(RepXCollection& src)
	{
		...
		{
			RenameSpringToStiffness(XmlReaderWriter& editor): RecursiveTraversal(editor) {}
		...
		{
			UpdateArticulationSwingLimit(XmlReaderWriter& editor): RecursiveTraversal(editor) {}
	...
	RepXCollection& RepXUpgrader::upgradeCollection(RepXCollection& src)
	{
		...
		else
		{
			if ( safeStrEq( srcVersion, "1.0" ) || safeStrEq( srcVersion, "3.1" ) )
			{
				...
				dest = &upgrade3_1CollectionTo3_2Collection( *dest );
				dest = &upgrade3_2CollectionTo3_3Collection( *dest );
			}
			else if ( safeStrEq( srcVersion, "3.1.1" ) )
			{
				dest = &upgrade3_1CollectionTo3_2Collection( src );
				dest = &upgrade3_2CollectionTo3_3Collection( *dest );
			}
			else if ( safeStrEq( srcVersion, "3.2.0" ) )
			{
				dest = &upgrade3_2CollectionTo3_3Collection( src );
			}
			else
				PX_ASSERT( false );
		}
