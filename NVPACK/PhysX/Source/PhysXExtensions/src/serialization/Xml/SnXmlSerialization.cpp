...
#undef	PX_REPX_DEFINE_ERROR_CODE
	...
	PxRepXErrorCode::Enum ReportError( PxRepXErrorCode::Enum errCode, const char* context, const char* file, int line )
	{
		...
		{
			Ps::ErrorHandler& handler = Ps::getFoundation().getErrorHandler();
			const char* message = GetRepXErrorString( errCode );
			handler.reportError(PxErrorCode::eINVALID_OPERATION, message, file, line);
			if ( NULL != context )
				handler.reportError(PxErrorCode::eDEBUG_INFO, context, file, line);
		}
	...
	public:
		...
		virtual PxRepXErrorCode::Enum instantiateCollection( PxRepXInstantiationArgs& inArgs, PxCollection* inCollection )
		{
			for ( PxU32 idx =0; idx < mCollection.size(); ++idx )
			{
				...
				if ( !theSerializer )
					REPX_REPORT_ERROR_RET( PxRepXErrorCode::eSerializerNotFound, theItem.liveObject.typeName );
				else
				{
					...
					if ( !theLiveObject.isValid() )
						REPX_REPORT_ERROR_RET( PxRepXErrorCode::eInvalidParameters, theLiveObject.typeName );
					else
					{
						if( inCollection )
						{
		...
		void load( PxFileBuf& inFileBuf, SerializationRegistry& s )
		{
	...
	bool PxSerialization::serializeCollectionToXml( PxOutputStream& outputStream, PxCollection& collection, PxSerializationRegistry& sr, PxCooking* cooking, const PxCollection* externalRefs, PxXmlMiscParameter* inArgs )
	{
		...
		if( nbObjects )
		{
			sortCollection( static_cast<Cm::Collection&>(collection), sn);
            ...
            for( PxU32 i = 0; i < nbObjects; i++ )
			{
				...
				if(id == PX_SERIAL_OBJECT_ID_INVALID)
					id = static_cast<const PxSerialObjectId>( reinterpret_cast<const size_t>( &s ));
