...
namespace
{
	...
	public:
		...
		{ 
			...
			PxI32 bytesToPad = Cm::getPadding(getTotalStoredSize(), alignment);
			enum { BUFSIZE = 64 };
			char buf[BUFSIZE];
			PxMemSet(buf, 0, bytesToPad < BUFSIZE ? bytesToPad : BUFSIZE);
			while(bytesToPad > 0)
			{
				writeData(buf, bytesToPad < BUFSIZE ? bytesToPad : BUFSIZE);
				bytesToPad -= BUFSIZE;
			}
	...
	void writeHeader(PxSerializationContext& stream)
	{
		struct Header
		{
			PxU32					header;
...
bool PxSerialization::serializeCollectionToBinary(PxOutputStream& outputStream, PxCollection& pxCollection, PxSerializationRegistry& sr, const PxCollection* pxExternalRefs, bool exportNames)
{
	...
	// write internal references
	{
		...
		for(InternalRefMap::Iterator iter = internalReferencesIdxMap.getIterator(); !iter.done(); ++iter)
			internalReferencesIdx[nbInternalIdxReferences++] = InternalReferenceIdx(to32(iter->first.first), iter->first.second, iter->second);
