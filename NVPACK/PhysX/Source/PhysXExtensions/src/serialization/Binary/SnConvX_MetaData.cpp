...
bool MetaData::load(PxInputStream& inputStream, MetaDataType type)
{
	...
	mFlip = false;
	{
		...
		if (!Sn::isBinaryPlatformTagValid(mPlatformTag))
		{
	...
	struct Local
	{
		static bool _computeDepth(const MetaData& md, MetaClass* current, int currentDepth, int& maxDepth)
		{
			...
			for(int i=0;i<nbBases;i++)
			{
				...
				if(!baseClass)
				{
					md.mConvX.displayMessage(PxErrorCode::eDEBUG_INFO, "Can't find class metadata", baseClassEntry.mName);	
					return false;
				}
...
{
	const PxSerializer* serializer = mSerializationRegistry.getSerializer(static_cast<PxType>(concreteType));
	PX_ASSERT(serializer);
	const char* className = serializer->getConcreteTypeName();

	if( !className )
	{
		displayMessage(PxErrorCode::eINTERNAL_ERROR, "Found non-supported type in getClassBySerialType(). Did you forget to update this code?");
		return NULL;
	}
	...
	if(type==META_DATA_SRC)
		metaClass = mMetaData_Src->getMetaClass(className);
	if(type==META_DATA_DST)
		metaClass = mMetaData_Dst->getMetaClass(className);

	if(!metaClass)
	{		
		displayMessage(PxErrorCode::eINTERNAL_ERROR, "Missing meta-data! You serialized a class without defining meta-data for it. Or you used an obsolete meta-data file.");
		return NULL;
	}	
