...
#define SERIAL_OBJECT_INDEX_TYPE_BIT (1u<<31)
		...
		typedef Ps::HashMap<InternalRefKey, SerialObjectIndex> InternalRefMap;
		...
		public:
			...
			{	
				...
				PxI32 bytesToPad = Cm::getPadding(mMemStream.getSize(), alignment);
				enum { BUFSIZE = 64 };
				char buf[BUFSIZE];
				PxMemSet(buf, 0, bytesToPad < BUFSIZE ? bytesToPad : BUFSIZE);
				while(bytesToPad > 0)
				{
					mMemStream.write(buf, bytesToPad < BUFSIZE ? bytesToPad : BUFSIZE);
					bytesToPad -= BUFSIZE;
				}
