		...
		const PxTransform& bB2w)
	{
		...
		if(driving & ((1<<PxD6Drive::eSLERP)|(1<<PxD6Drive::eSWING)|(1<<PxD6Drive::eTWIST)))
		{
			...
			if(driving & (1<<PxD6Drive::eSLERP))
			{
				...
				PxVec3 axis[3] = { PxVec3(1,0,0), PxVec3(0,1,0), PxVec3(0,0,1) };
				...
				for(PxU32 i = 0; i < 3; i++)
					g.angular(axis[i], axis[i].dot(velTarget), -delta.getImaginaryPart()[i], drives[PxD6Drive::eSLERP], PxConstraintSolveHint::Enum(258));
			}
			else 
			{
