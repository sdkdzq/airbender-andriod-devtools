...
void computeWheelCenterOffsetsTank(const PxF32 wheelFrontZ, const PxF32 wheelRearZ, const PxVec3& chassisDims, const PxF32 wheelWidth, const PxF32 wheelRadius, const PxU32 numWheels, PxVec3* wheelCentreOffsets)
{
...
PxVehicleDriveTank* createVehicleTank(const VehicleDesc& tankDesc, PxPhysics* physics, PxCooking* cooking)
{
	...
	PxVehicleWheelsSimData* wheelsSimData = PxVehicleWheelsSimData::allocate(numWheels);
	{
		...
		computeWheelCenterOffsetsTank(frontZ, rearZ, chassisDims, wheelWidth, wheelRadius, numWheels, wheelCentreOffsets);
		...
		PxVehicleWheelData wheels[PX_MAX_NB_WHEELS];
		{
			...
			for(PxU32 i = 0; i < numWheels; i++)
			{
				wheels[i].mMass = tankDesc.wheelMass;
				wheels[i].mMOI = tankDesc.wheelMOI;
		...
		PxVehicleSuspensionData suspensions[PX_MAX_NB_WHEELS];
		{
			...
			PxVehicleComputeSprungMasses(numWheels, wheelCentreOffsets, tankDesc.chassisCMOffset, tankDesc.chassisMass, 1, suspSprungMasses);
		...
		PxVec3 tireForceAppCMOffsets[PX_MAX_NB_WHEELS];
		{
			for(PxU32 i = 0; i < numWheels; i++)
			{
				...
				wheelCentreCMOffsets[i] = wheelCentreOffsets[i] - tankDesc.chassisCMOffset;
