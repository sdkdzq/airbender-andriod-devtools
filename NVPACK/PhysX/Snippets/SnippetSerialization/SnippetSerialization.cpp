...
void* createAlignedBlock(PxInputData& data, void* baseAddr)
{
	PxU32 length = data.getLength();
	baseAddr = malloc(length+PX_SERIAL_FILE_ALIGN-1);
...
void cleanupPhysics(bool interactive)
{
	...
	if(gConnection != NULL)
		gConnection->release();
	gPhysics->release();	// disposes of all objects	
	profileZoneManager->release();

	free(gSharedBlock);		// now the objects are disposed of, it's safe to release the space they occupy
	free(gActorBlock);
