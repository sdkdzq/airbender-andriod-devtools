...
#define TERNARY_OP_HALF_MACRO(name) do {\
   __half val; \
   asm( "{.reg .b16 af,bf,cf,df;\n" \
        "mov.b16 af,%1;\n" \
        "mov.b16 bf,%2;\n" \
        "mov.b16 cf,%3;\n" \
        ""#name".f16 df,af,bf,cf;\n" \
        "mov.b16 %0,df;}\n" \
...
#endif /*defined(__local_warpSize)*/
...
#undef __LDG_PTR
#endif /*defined(__cplusplus) && __CUDA_ARCH__ >= 320*/
__CUDA_FP16_DECL__ __half __float2half(const float f)
{
   __half val;
   asm volatile("{ .reg .b16 temp;\n"
                "cvt.rn.f16.f32 temp, %1;\n"
                "mov.b16 %0,temp;}\n"
                : "=h"(val.x) : "f"(f));
   return val;
}
